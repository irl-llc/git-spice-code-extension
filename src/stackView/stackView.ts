...
class StackView {
	...
	private handleMessage(message: ExtensionMessage): void {
		...
		if (message.type === 'commitFiles') this.handleCommitFiles(message.sha, message.files);
		if (message.type === 'branchFiles') this.handleBranchFiles(message.branchName, message.files);
	}
	...
	private updateRepoSection(repo: RepositoryViewModel, force?: boolean): void {
		...
		this.updateUntrackedCard(branchList, repo);
	...
	private createCommitsContainer(branch: BranchViewModel, _card: HTMLElement, view: RepoViewState): HTMLElement {
		return renderCommitsContainer(branch, view.commitState, this.getPostMessage(), animations, this.getTreeColors());
	}

	private createSummaryContainer(branchName: string, view: RepoViewState): HTMLElement {
		return renderBranchSummary(branchName, view.branchSummaryState, this.getPostMessage());
	}

	// --- Untracked Branch ---

	private updateUntrackedCard(branchList: HTMLElement, repo: RepositoryViewModel): void {
		branchList.querySelector('.untracked-item')?.remove();
		if (!repo.untrackedBranch) return;

		const card = renderUntrackedCard(repo.untrackedBranch, this.getPostMessage());
	...
	private insertUncommittedWithTree(
		...
		const newCard = renderUncommittedCard(
			repo.uncommitted!, repo.uncommittedTreeFragment!, this.getTreeColors(), view.workingCopyState, this.getPostMessage(),
		);
	...
	private insertUncommittedWithoutTree(
		...
		const card = renderTreelessUncommittedCard(uncommitted, view.workingCopyState, this.getPostMessage());
	...
	/** Handles commit file responses — searches across all repo sections. */
	...
	/** Handles branch file responses — searches across all repo sections. */
