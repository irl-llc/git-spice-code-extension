/**
 * Pure semver-derivation logic for the scheduled auto-release (issue #30).
 *
 ...
 *
 * Versioning policy (owner's binding decision on #30): the extension is pre-1.0,
 * so we stay on `0.x`. Per semver-for-0.x, a feature OR a breaking change bumps
 * the MINOR component (we never bump major while on `0.x`); a bug fix bumps the
 * PATCH component. Docs-only changes never produce a fragment, so they never
 * release.
 */
