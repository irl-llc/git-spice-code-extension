...
6. **Feature coverage policy — every feature ships a verified screenshot**:
   ...
   - **Remote/forge-dependent features MUST be driven by the `shamhub` fake forge** (`.gs/src/internal/forge/shamhub/`), not by mocking. Anything that reads or writes a remote — Change Request status, PR **comment counts**, `stack submit`, `repo sync` — must seed shamhub with the relevant PRs/comments/state and assert the rendered result, including the failure path (e.g. fetch error, unauthenticated). Mocking is not acceptable here; simulate the real forge end-to-end. **Note:** shamhub is not yet wired into the test harness (see [[reference_shamhub]]) — wiring it is a prerequisite for, and part of, any comment-count / CR / submit / sync screenshot coverage.
