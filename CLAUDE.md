...
4. **Pinned `gs` binary for tests** (`npm run gs:fetch`):
   ...
   - **Stock binary for gating tests** (issue #72): `npm run gs:fetch` ALSO builds a VANILLA upstream `abhinav/git-spice` binary, pinned to the release tag in [.gs-stock-version](.gs-stock-version), into `.gs/bin/gs-stock` (sources in `.gs/src-stock/`, build marker `.gs/.built-stock-tag`). It carries no shamhub registration and no beta integration patches — exactly what a stock install runs. Tests point at it via the `GIT_SPICE_STOCK_BIN` env var to prove the extension's beta surfaces (the `integration` command group and the forge-status `gs ll -S` path) stay gated OFF against stock git-spice. State-only coverage lives in [src/test/unit/stockBinary.test.ts](src/test/unit/stockBinary.test.ts), which SKIPS when the binary is not built so `test:unit` still passes on a fresh checkout. CI sets `GIT_SPICE_STOCK_BIN=${{ github.workspace }}/.gs/bin/gs-stock`. To bump: edit the tag in `.gs-stock-version` and re-run `npm run gs:fetch`.
...
5. **Visual snapshot tests** (`npm run test:e2e:playwright:docker`):
   ...
   - **Two viewing modes for snapshots:**
     ...
     - **SCM sub-pane** ([treeFragment.spec.ts](src/test/e2e/playwright/treeFragment.spec.ts)) — opens the view in the SCM sidebar for narrow-column captures focused on the tree-fragment SVG geometry.
     - Helpers in [fixtures/webview.ts](src/test/e2e/playwright/fixtures/webview.ts): `openGitSpiceView` (sidebar), `openGitSpiceEditor` (editor tab), `setSidebarWidth(workbench, px)` (drag sash), `collapseSidebarSiblings(workbench, keepTitle)` (free vertical space).
   - **Adding a new snapshot test**: prefer the full-pane mode unless you specifically want to capture the narrow-column rendering. Copy [src/test/e2e/playwright/fullPaneCards.spec.ts](src/test/e2e/playwright/fullPaneCards.spec.ts) — seed the gs repo to the visual scenario, call `openGitSpiceEditor`, locate the element, assert `toHaveScreenshot('descriptive-name.png')`. Stability helpers (animations off, fonts ready) are applied inside the open helpers.

6. **Feature coverage policy — every feature ships a verified screenshot**:
